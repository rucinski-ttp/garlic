...
int i2c_async_init(void) { return 0; }
...
int i2c_async_write_read(uint16_t addr,
...
int i2c_blocking_write_read(uint16_t addr,
                            const uint8_t *wdata, size_t wlen,
                            uint8_t *rdata, size_t rlen,
                            int timeout_ms)
{
...
int i2c_bus_recover(void) { return 0; }

int i2c_ping(uint16_t addr) { return ((addr & 0x7F) == g_present_addr) ? 0 : -ENODEV; }
