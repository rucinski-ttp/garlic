...
extern "C" void command_registry_init(void);

extern "C" void command_register_builtin(void);
...
TEST(CommandHandlers, GitVersion)
{
    ...
    uint16_t st = 0;
    ASSERT_TRUE(command_dispatch(CMD_ID_GET_GIT_VERSION, nullptr, 0, out, &out_len, &st));
...
TEST(CommandHandlers, Uptime)
{
    ...
    uint16_t st = 0;
    ASSERT_TRUE(command_dispatch(CMD_ID_GET_UPTIME, nullptr, 0, out, &out_len, &st));
    EXPECT_EQ(st, CMD_STATUS_OK);
...
TEST(CommandHandlers, FlashRead)
{
    ...
    uint16_t st = 0;
    ASSERT_TRUE(command_dispatch(CMD_ID_FLASH_READ, req, sizeof(req), out, &out_len, &st));
    EXPECT_EQ(st, CMD_STATUS_OK);
...
TEST(CommandHandlers, Echo)
{
    ...
    uint8_t out[16]; size_t out_len = sizeof(out);
    uint16_t st = 0;
    ASSERT_TRUE(command_dispatch(CMD_ID_ECHO, req, sizeof(req), out, &out_len, &st));
    EXPECT_EQ(st, CMD_STATUS_OK);
