...
protected:
    ...
    void SetUp() override {
        ...
        grlc_cb_init(&cb, buffer, BUFFER_SIZE);
    }
...
TEST_F(CircularBufferTest, InitializationTest) {
    ...
    EXPECT_TRUE(grlc_cb_is_empty(&cb));
    EXPECT_FALSE(grlc_cb_is_full(&cb));
}

TEST_F(CircularBufferTest, InitializationWithNullPointers) {
    circular_buffer_t cb_test;
    EXPECT_EQ(grlc_cb_init(nullptr, buffer, BUFFER_SIZE), -1);
    EXPECT_EQ(grlc_cb_init(&cb_test, nullptr, BUFFER_SIZE), -1);
    EXPECT_EQ(grlc_cb_init(&cb_test, buffer, 0), -1);
}
...
TEST_F(CircularBufferTest, WriteAndReadSingleByte) {
    ...
    EXPECT_EQ(grlc_cb_write(&cb, &write_data, 1), 1u);
    EXPECT_FALSE(grlc_cb_is_empty(&cb));
    EXPECT_EQ(grlc_cb_available(&cb), 1u);

    EXPECT_EQ(grlc_cb_read(&cb, &read_data, 1), 1u);
...
TEST_F(CircularBufferTest, WriteAndReadMultipleBytes) {
    ...
    EXPECT_EQ(grlc_cb_write(&cb, write_data, 5), 5u);
    EXPECT_EQ(grlc_cb_available(&cb), 5u);

    EXPECT_EQ(grlc_cb_read(&cb, read_data, 5), 5u);
...
TEST_F(CircularBufferTest, PeekDoesNotRemoveData) {
    ...
    grlc_cb_write(&cb, write_data, 3);

    EXPECT_EQ(grlc_cb_peek(&cb, peek_data, 3), 3u);
    EXPECT_THAT(std::vector<uint8_t>(peek_data, peek_data + 3),
                ElementsAre(0xAA, 0xBB, 0xCC));
    EXPECT_EQ(grlc_cb_available(&cb), 3u);

    EXPECT_EQ(grlc_cb_read(&cb, read_data, 3), 3u);
...
TEST_F(CircularBufferTest, WrapAroundWrite) {
    ...
    grlc_cb_write(&cb, initial_data.data(), initial_data.size());
    
    // Read some data to make room at the beginning
    std::vector<uint8_t> dummy(20);
    grlc_cb_read(&cb, dummy.data(), 20);

    // Write data that will wrap around
    std::vector<uint8_t> wrap_data(30, 0x22);
    size_t written = grlc_cb_write(&cb, wrap_data.data(), wrap_data.size());
    ...
    size_t read_count = grlc_cb_read(&cb, read_all.data(), BUFFER_SIZE);
...
TEST_F(CircularBufferTest, FullBufferBehavior) {
    ...
    size_t written = grlc_cb_write(&cb, data.data(), data.size());
    EXPECT_EQ(written, BUFFER_SIZE - 1);
    EXPECT_TRUE(grlc_cb_is_full(&cb));

    // Try to write more - should fail
    uint8_t extra = 0x44;
    EXPECT_EQ(grlc_cb_write(&cb, &extra, 1), 0u);
}

TEST_F(CircularBufferTest, GetReadBlock) {
    uint8_t *data_ptr = nullptr;
    size_t len = 0;

    // Test empty buffer
    EXPECT_EQ(grlc_cb_get_read_block(&cb, &data_ptr, &len), -1);

    // Write some data
    std::vector<uint8_t> data(50, 0x77);
    grlc_cb_write(&cb, data.data(), data.size());

    // Get read block
    EXPECT_EQ(grlc_cb_get_read_block(&cb, &data_ptr, &len), 0);
    EXPECT_EQ(len, 50u);
    EXPECT_EQ(data_ptr[0], 0x77);

    // Advance and verify
    grlc_cb_advance_read(&cb, 25);
    EXPECT_EQ(grlc_cb_available(&cb), 25u);
}
...
TEST_F(CircularBufferTest, GetWriteBlockLinear) {
    ...
    EXPECT_EQ(grlc_cb_get_write_block(&cb, &data_ptr, &len), 0);
    EXPECT_EQ(len, BUFFER_SIZE - 1);  // Keeps one byte free

    // Write directly to the block
    memset(data_ptr, 0x88, 100);
    grlc_cb_advance_write(&cb, 100);
    EXPECT_EQ(grlc_cb_available(&cb), 100u);

    // Verify data
    std::vector<uint8_t> read_data(100);
    grlc_cb_read(&cb, read_data.data(), 100);
...
TEST_F(CircularBufferTest, GetBlocksWithWrapAround) {
    ...
    size_t read_len = 0;
    EXPECT_EQ(grlc_cb_get_read_block(&cb, &read_ptr, &read_len), 0);
    EXPECT_EQ(read_len, 10u);  // Only until end of buffer

    // Advance and get next block
    grlc_cb_advance_read(&cb, read_len);
    EXPECT_EQ(grlc_cb_get_read_block(&cb, &read_ptr, &read_len), 0);
...
TEST_F(CircularBufferTest, ConcurrentWriteRead) {
    ...
    std::thread writer([&]() {
        uint8_t data = 0;
        while (!stop) {
            size_t written = grlc_cb_write(&cb, &data, 1);
            if (written > 0) {
                total_written++;
                data++;
            }
    ...
    std::thread reader([&]() {
        uint8_t data;
        uint8_t expected = 0;
        while (!stop || grlc_cb_available(&cb) > 0) {
            size_t read = grlc_cb_read(&cb, &data, 1);
            if (read > 0) {
...
TEST_F(CircularBufferTest, PartialReadWrite) {
    ...
    size_t written = grlc_cb_write(&cb, big_data.data(), big_data.size());
    EXPECT_EQ(written, BUFFER_SIZE - 1);  // Should only write what fits

    // Try to read more than available
    std::vector<uint8_t> read_data(BUFFER_SIZE * 2);
    size_t read = grlc_cb_read(&cb, read_data.data(), read_data.size());
...
TEST_F(CircularBufferTest, InvalidParameters) {
    ...
    EXPECT_EQ(grlc_cb_write(nullptr, &data, 1), 0u);
    EXPECT_EQ(grlc_cb_write(&cb, nullptr, 1), 0u);
    EXPECT_EQ(grlc_cb_write(&cb, &data, 0), 0u);

    EXPECT_EQ(grlc_cb_read(nullptr, &data, 1), 0u);
    EXPECT_EQ(grlc_cb_read(&cb, nullptr, 1), 0u);
    EXPECT_EQ(grlc_cb_read(&cb, &data, 0), 0u);

    EXPECT_EQ(grlc_cb_available(nullptr), 0u);
    EXPECT_EQ(grlc_cb_free_space(nullptr), 0u);
}
